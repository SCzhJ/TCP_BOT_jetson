		...
		while (ros::ok())
		{
			...
			else
			{
				if (pubScan)
				{
					...
					// Iterate through the laser scans, if the point x coordinate is whithin [-threshold, threshold]
					// And y coordinate is within [-threshold, threshold], then replace it with inf

					// Calculate the angle of the current scan from the start angle
					for (size_t i = 0; i < msg.ranges.size(); ++i) {
						float angle = msg.angle_min + i * msg.angle_increment;

						// Calculate the x and y coordinates of the scanned point
						float x = msg.ranges[i] * cos(angle);
						float y = msg.ranges[i] * sin(angle);

						// If the x and y coordinates are within the threshold, replace the range with infinity
						if ((x < 0.17 && x > -0.15) && abs(y) < 0.17) {
